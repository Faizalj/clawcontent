...
```bash
git clone https://github.com/Faizalj/clawcontent.git
cd clawcontent
bun install
bun link          # Install "clawcontent" CLI globally
bun run dev       # Start dashboard at http://localhost:3456
```

After `bun link`, use CLI from anywhere: `clawcontent channels`

### OpenClaw Skill (optional)

```bash
mkdir -p ~/.openclaw/workspace/skills/clawcontent
cp skill/SKILL.md ~/.openclaw/workspace/skills/clawcontent/
openclaw gateway restart
```

Now OpenClaw agents can control ClawContent via chat.
